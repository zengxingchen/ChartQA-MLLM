...
def extract_and_save_data(plot_code, csv_path):
    ...
    if data_match:
        ...
        print(f"No data found in the code for {csv_path}")
...
def generate_diversified_vega_lite_code_echart(file_path, save_dir, chart_type, method):
    client = OpenAI(base_url='https://api.chsdw.top/v1', api_key='sk-OqykAM7nuMwy5UL05cEbD6Ba09534b2dBf636aF53eA2C78e')
    ...
    prompt_messages = [
        ...
        {"role": "user", "content": "Given the following echart code of a {chart_type}: " + original_vl_code +
         " Please generate a diversified version of this echart code of a {chart_type}. Here are some options you should follow:\
    ...
    try:
        ...
        if csv_data and isinstance(csv_data[0], str):
            ...
            file_path = os.path.join(save_dir, f'_14_{filename_pre}.csv')
        ...
        diversified_json_path = os.path.join(save_dir, f"_14_{filename}")
...
def generate_diversified_vega_lite_code(file_path, save_dir, chart_type, method):
    ...
    random.shuffle(topics_pool)
    prompt_messages = [
        {"role": "system", "content": "You are a highly intelligent AI familiar with data visualization and {method}."},
        {"role": "user", "content": "Given the following {method} code of a {chart_type}: " + original_vl_code +
         " Please generate a diversified version of this {method} code of {chart_type}. Here are some options you should follow:\
            1. Generate data points that fit the {chart_type} (not too much datapoint), but you should not skip any data point in your code. Do not add any comment in your code\
            2. Add or change some data point with new corresponding values to enrich the visualization.\
            print the final table data you use in the code within triple backticks (```), and don't include things like csv, plaintext in the triple backticks (```).\
            3. Modify the color scheme using specific color codes (e.g., #RRGGBB) for better clarity or visual appeal. Avoid using color categories.\
            4. change width and height of the chart reasonably\
    ...
    try:
        ...
        vega_lite_code_blocks = re.findall(r'```(.*?)```', assistant_reply, re.DOTALL)
        ...
        csv_lines = table.split('\n')
        filename_pre = filename.split('.')[0]
        file_path = os.path.join(save_dir, f'_3_{filename_pre}.csv')
        ...
        # code
        vega_lite_code = '\n'.join(vega_lite_code_blocks[1].strip().splitlines()[1:]) # 删掉第一行python
        print(vega_lite_code)
        diversified_json_path = os.path.join(save_dir, f"_3_{filename}")
        with open(diversified_json_path, 'w', encoding='utf-8') as file:
            file.write(vega_lite_code)
        print(f'Saved diversified python code to {diversified_json_path}')
    except Exception as e:
        if "429" in str(e):  # Check if the error is 429
            wait_time = 5  # 等待时间
            time.sleep(wait_time)
        # print(f"Failed to generate diversified code for {filename}: {e}")

def main(data_dir, save_dir, chart_type, method):
    os.makedirs(save_dir, exist_ok=True)
    if method == "echart":
        file_paths = [os.path.join(data_dir, f) for f in os.listdir(data_dir) if f.endswith('.html')]
        for file_path in tqdm(file_paths, desc='Processing files'):
            # Echart 相关处理
            generate_diversified_vega_lite_code_echart(file_path, save_dir, chart_type, method)
    else:
        file_paths = [os.path.join(data_dir, f) for f in os.listdir(data_dir) if f.endswith('.py')]
        for file_path in tqdm(file_paths, desc='Processing files'):
            generate_diversified_vega_lite_code(file_path, save_dir, chart_type, method)
...
if __name__ == "__main__":
    ...
    main(data_dir=args.data_dir, save_dir=args.save_dir, chart_type=args.chart_type, method=args.method)
